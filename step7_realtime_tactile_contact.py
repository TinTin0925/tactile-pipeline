...
from vggt_step_common import DEFAULT_HAND_EYE, DEFAULT_RUNS_DIR, DEFAULT_SOURCE_DIR
...
def default_pose_json() -> Path:
    return DEFAULT_SOURCE_DIR.parent / "current_T_base_tool.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hand-eye-key", default="T_tail_in_cam",
                        help="Transform used as T_tool_camera. Keep this matched to the Step3A key that worked.")
    ...
    parser.add_argument("--camera-to-tactile-translation-scale", type=float, default=0.001,
                        help="Use 0.001 when T_camera_tactile translation is in mm; use 1.0 if it is already meters.")
...
def main() -> None:
    ...
    if args.tool_tactile:
        ...
        tool_tactile_source = f"{args.hand_eye.resolve()}::{args.hand_eye_key} @ {args.camera_to_tactile.resolve()}"
    ...
    if log_path is None:
        ...
        log_path = DEFAULT_RUNS_DIR / "step5_tactile_contact" / f"contact_log_{stamp}.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)

    print("[Step5] cloud:", cloud_path)
    print("[Step5] points:", len(points))
    print("[Step5] T_tool_tactile source:", tool_tactile_source)
    print("[Step5] threshold:", args.threshold, "m")
    print("[Step5] log:", log_path)
    if args.replay_jsonl:
        print("[Step5] replay:", args.replay_jsonl.resolve())
    else:
        print("[Step5] live pose json:", args.pose_json.resolve())
    ...
    if not args.no_visualize:
        ...
        vis.create_window(window_name="Step5 tactile contact", width=1280, height=820)
        vis.add_geometry(pcd)
        render = vis.get_render_option()
        render.point_size = args.point_size
        render.background_color = np.array([0.02, 0.02, 0.02])
        tip_sphere = make_sphere(args.sphere_radius, [1.0, 0.6, 0.0])
        near_sphere = make_sphere(args.sphere_radius * 0.75, [0.1, 0.45, 1.0])
        ...
        while True:
            if replay_iter is not None:
                ...
                if not pose_path.is_file():
                    print(f"[Step5] waiting for pose file: {pose_path}")
            ...
            if last_state is None or in_contact != last_state or args.one_shot:
                ...
                print(f"[Step5] {state}: distance={distances[0] * 1000.0:.2f} mm, tip={p_tip}")
            ...
            if vis and tip_sphere is not None and near_sphere is not None:
                tip_sphere.translate(p_tip - last_tip, relative=True)
                near_sphere.translate(nearest - last_near, relative=True)
                tip_sphere.paint_uniform_color([1.0, 0.05, 0.05] if in_contact else [1.0, 0.6, 0.0])
